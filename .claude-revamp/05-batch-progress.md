...
2026-05-04T19:00Z | buyer-signin | STOPPED | retry #2: STATUS.md Resolution filled in by operator + _batch-7-schema-plan.md landed. Ran the §3.4 sequencing check — Batch 5 migration 0012 NOT applied to dev (postal_code/province/business_name still deferred in Drizzle per addresses.ts:20-25 + auth.ts:20-24); addresses.landmark missing entirely (plan §3.3 calls for Batch 5 amendment that hasn't shipped). Per the schema plan's hard-stop rule, runner cannot begin implementation. See STATUS.md "2026-05-04 retry attempt #2".
2026-05-04T20:00Z | buyer-signin | STOPPED | retry #3: operator partially undeferred Drizzle declarations (uncommitted M on addresses.ts + auth.ts now declare postal_code/province/landmark + business_name) but did NOT author migration 0013 for addresses.landmark (journal still ends idx 12) and did NOT update STATUS.md Resolution section. Drizzle now declares a column the DB cannot have — strictly worse than retry #2. Schema plan §3.4 STOP repeats. See STATUS.md "2026-05-04 retry attempt #3".
2026-05-04T22:00Z | buyer-signin | SHIPPED | f5c1f30 (build green; smoke deferred — auth flow needs Twilio + dev DB, same defer pattern as Batches 5/6. Lands +92 phone-chip / brand-cluster / mobile app-bar molecules consumed across the rest of Batch 7; retires AuthModal + openAuthModal; cart-store persist v3 adds guestSessionId for OQ-G(b))
2026-05-04T22:30Z | buyer-otp | SHIPPED | 7e6f8ea (build green; smoke deferred same as buyer-signin. Lands OtpGrid 6-box molecule + OtpHelpDialog; replaces legacy single-input form; resend cap 3 + lockout heuristic on better-auth allowedAttempts:3)
2026-05-04T23:00Z | buyer-signup-generic | SHIPPED | d2464d8 (build green; migration 0013 authored — apply on dev DB still pending operator. Lands /sign-up?type=generic with full generic-variant UI + sessionStorage pendingSignup hand-off + POST /api/auth/post-signup hook. Shopkeeper zod variant defined ahead of its UI (single discriminated union per Q11); shopkeeper UI lands next commit. EN/اردو toggle hidden per OQ-I.)
2026-05-04T23:30Z | buyer-signup-shopkeeper | SHIPPED | 00d08ab (build green; migration 0014 authored — apply on dev DB pending. Lands /sign-up?type=shopkeeper end-to-end, user.shopName + user.shopAddress columns + better-auth additionalFields + mobile-only mart-shelf strip with lucide silhouettes per OQ-A. EN-only per OQ-I.)
2026-05-05T00:00Z | buyer-checkout-one-time-addr | SHIPPED | d428b17 (build green; migrations 0015 + 0016 authored — apply on dev DB pending. Lands the one-time delivery card + OQ-G(b) guest checkout end-to-end. Schema: orders.user_id NULLable + guest_session_id + shipping_postal_code + shipping_province; @repo/constants/geo with pakistanProvinceEnum; shippingAddressSchema + checkoutCartPayloadSchema extended. Server: /api/checkout relaxes requireSession to session-or-guest, snapshots postal+province, supports atomic saveAddress; /api/user/addresses POST stops silently dropping postal+province. UI: OneTimeDeliveryCard with Don't save toggle, hidden-for-guests, mutual-exclusion with saved-address radio, Sign-in deep-link in guest hint. Landmark dropped per Q6(d).)
