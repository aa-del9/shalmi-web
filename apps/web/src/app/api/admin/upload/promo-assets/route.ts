...
import { PROMO_ASSETS_BUCKET } from '@repo/storage';
...
export async function POST(req: NextRequest) {
  ...
  try {
    const results = await Promise.all(
      files.map((file) => uploadImageToStorage(file, PROMO_ASSETS_BUCKET))
    );
    return jsonSuccess({ results });
  } catch (err) {
    console.error('Admin promo-assets upload error:', err);
